...
fn return_ordered(rules: Vec<(u32, u32)>, pages: Vec<Vec<u32>>) -> Vec<Vec<u32>> {
    ...
    let mut ok_pages: Vec<_> = pages
        ...
        .collect();
let sorted_results: Vec<_> = ok_pages
    .iter_mut()
    .map(|update| {
        update.sort_by(|a, b| {
            match (rules.iter().any(|(before, after)| *before == *a && *after == *b),
                   rules.iter().any(|(before, after)| *before == *b && *after == *a)) {
...
#[cfg(test)]
mod tests {
    use crate::part2::{parse_rules, sum_middle_values};
